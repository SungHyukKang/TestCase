...
//			}
			...
			// authenticate() 함수가 호출되면 인증 프로바이더가 UserDetailService의 loadUserByUsername(토큰의
			// 첫번째 파라미터)을 호출하고
			// UserDetails를 리턴 받아서 토큰의 두번째 파라미터(credential)과 UserDetails(DB값)의
			// getPassword()와 비교해서 동일하면
	...
	protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain,
		...
		String jwtToken = JWT.create().withSubject("토큰")
				.withExpiresAt(new Date(System.currentTimeMillis() + JwtProperties.EXPIRATION_TIME))
				.withClaim("id", principalDetails.getUser().getId())
				.withClaim("username", principalDetails.getUser().getUsername())
				.sign(Algorithm.HMAC512(JwtProperties.SECRET));

		String refToken = JWT.create().withSubject("토큰")
